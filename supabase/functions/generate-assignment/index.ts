...
serve(
  withCors(async (req) => {
    ...
    if (!parsed.success) return Errors.invalidRequest("Missing topic", requestId, req);
    ...
    const base = slugify(topic) || "assignment";
    ...
    if (inserted.error || !inserted.data?.id) {
      return Errors.internal(inserted.error?.message || "Failed to enqueue assignment generation job", requestId, req);
    }

    return { ok: true, jobId: inserted.data.id, courseId, topic };
  }),
);
