...
Deno.serve(withCors(async (req: Request) => {
  ...
  try {
    ...
    const effectiveJobId = (typeof jobId === "string" && jobId.trim()) ? jobId.trim() : "__editor_variants_audit__";
    if (!courseId || typeof courseId !== 'string') {
      return Errors.invalidRequest("Invalid courseId", reqId, req) as any;
    }

    // 1) generate-variants-audit
    const genRes = await fetch(`${SUPABASE_URL}/functions/v1/generate-variants-audit`, {
      method: 'POST',
    ...
    const applyRes = await fetch(`${SUPABASE_URL}/functions/v1/apply-job-result`, {
      method: 'POST',
      headers: { 'X-Agent-Token': AGENT_TOKEN, 'Content-Type': 'application/json' },
      body: JSON.stringify({ jobId: effectiveJobId, courseId, mergePlan, description: 'Editor Variants Audit', dryRun: !apply }),
    });
    ...
    return new Response(JSON.stringify({ ok: true, courseId, preview: applyJson?.preview, etag: applyJson?.etag, applied: apply === true, report: genJson?.report }), {
      status: 200,
