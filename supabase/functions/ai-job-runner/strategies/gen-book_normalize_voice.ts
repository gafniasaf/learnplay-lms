...
export class GeneratedBookNormalizeVoice implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Rewrite the generated book content to match the N3 (student-friendly) voice while preserving structure and correctness. Return JSON {"bookId": string, "bookVersionId": string}.`;
    // Basic interpolation
    if (payload) {
