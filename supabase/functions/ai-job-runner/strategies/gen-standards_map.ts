...
export class GeneratedStandardsMap implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Map the ingested material {{material_id}} to the standards document {{standards_document_id}} using embeddings. Return JSON {"mapping_id": string, "matches": number}.`;
    // Basic interpolation
    if (payload) {
