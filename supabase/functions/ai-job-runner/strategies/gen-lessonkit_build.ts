...
export class GeneratedLessonkitBuild implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Build a grounded lesson kit from the provided material/course context. Return JSON {"lesson_kit": json, "status": "ready"|"failed", "notes": string}.`;
    // Basic interpolation
    if (payload) {
