...
export class GeneratedMesCorpusIndex implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Index MES corpus documents into org-scoped embeddings for semantic search. Return JSON {"indexed": number}.`;
    // Basic interpolation
    if (payload) {
