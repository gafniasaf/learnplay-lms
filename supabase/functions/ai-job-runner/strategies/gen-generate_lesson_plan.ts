...
export class GeneratedGenerateLessonPlan implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Generate a grounded lesson plan for the teacher request in payload.queryText. Return JSON {"ok": boolean, "answer": string, "lessonPlan": json, "recommendations": json[]}.`;
    // Basic interpolation
    if (payload) {
