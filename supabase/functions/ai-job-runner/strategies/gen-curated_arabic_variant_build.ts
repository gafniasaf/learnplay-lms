...
export class GeneratedCuratedArabicVariantBuild implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Generate an Arabic (RTL) variant of a pre-rendered curated material pack. Arabic output must include Dutch keywords in blue parentheses for terminology learning. Return JSON {"ok": boolean, "curated_material_id": string, "language_variant": "ar"}.`;
    // Basic interpolation
    if (payload) {
