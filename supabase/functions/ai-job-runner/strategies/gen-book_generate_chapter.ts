...
export class GeneratedBookGenerateChapter implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Generate content for a single chapter in a skeleton-first book pipeline (no image generation). Return JSON {"bookId": string, "bookVersionId": string, "chapterIndex": number}.`;
    // Basic interpolation
    if (payload) {
