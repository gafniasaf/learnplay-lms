...
export class GeneratedDraftAssignmentPlan implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `You are an adaptive learning coach. Given {{title}}, {{subject}}, {{learner_id}}, and recent session context, propose the next best set of activities. Return JSON {"summary": string, "suggested_actions": string[], "updated_fields": {"ai_variant_id": string, "status": string}}.`;
    // Basic interpolation
    if (payload) {
