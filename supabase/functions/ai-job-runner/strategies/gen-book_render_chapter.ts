...
export class GeneratedBookRenderChapter implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Enqueue a chapter render job for a canonical book version. Return JSON {"run_id": string, "chapter_index": number}.`;
    // Basic interpolation
    if (payload) {
