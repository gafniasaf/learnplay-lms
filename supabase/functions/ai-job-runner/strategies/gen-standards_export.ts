...
export class GeneratedStandardsExport implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Export the standards mapping {{mapping_id}} to CSV and return JSON {"export_path": string}.`;
    // Basic interpolation
    if (payload) {
