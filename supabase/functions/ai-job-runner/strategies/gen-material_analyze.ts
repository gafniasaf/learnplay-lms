...
export class GeneratedMaterialAnalyze implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Analyze the ingested material and produce a teacher-facing summary plus recommended next actions. Return JSON {"summary": string, "key_concepts": string[], "suggested_assignments": string[]}.`;
    // Basic interpolation
    if (payload) {
