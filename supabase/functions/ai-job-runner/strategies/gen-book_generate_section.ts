...
export class GeneratedBookGenerateSection implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Generate content for a single section in a skeleton-first book pipeline (internal). Return JSON {"bookId": string, "bookVersionId": string, "chapterIndex": number, "sectionIndex": number}.`;
    // Basic interpolation
    if (payload) {
