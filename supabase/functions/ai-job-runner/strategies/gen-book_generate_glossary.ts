...
export class GeneratedBookGenerateGlossary implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Generate a glossary / begrippenlijst for a canonical book version. Return JSON {"bookId": string, "bookVersionId": string}.`;
    // Basic interpolation
    if (payload) {
