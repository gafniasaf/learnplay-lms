...
export class GeneratedBookIngestVersion implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Create a versioned canonical book record from uploaded inputs. Return JSON {"book_version_id": string, "canonical_path": string}.`;
    // Basic interpolation
    if (payload) {
