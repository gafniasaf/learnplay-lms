...
export class GeneratedGenerateMultiWeekPlan implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Generate a multi-week lesson plan based on payload.queryText. Return JSON {"ok": boolean, "answer": string, "multiWeekPlan": json, "recommendations": json[]}.`;
    // Basic interpolation
    if (payload) {
