...
export class GeneratedMaterialIngest implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Ingest the uploaded material at {{storage_path}} into chunks and embeddings. Return JSON {"material_id": string, "chunk_count": number}.`;
    // Basic interpolation
    if (payload) {
