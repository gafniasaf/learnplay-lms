...
export class GeneratedBookGenerateFull implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Generate a full book using a skeleton-first pipeline (no image generation). Return JSON {"bookId": string, "bookVersionId": string, "chapterCount": number}.`;
    // Basic interpolation
    if (payload) {
