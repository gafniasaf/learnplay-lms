...
export class GeneratedBookGenerateIndex implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Generate an index term list for a canonical book version (no page numbers yet). Return JSON {"bookId": string, "bookVersionId": string}.`;
    // Basic interpolation
    if (payload) {
