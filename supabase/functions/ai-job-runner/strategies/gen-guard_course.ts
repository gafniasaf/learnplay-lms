...
export class GeneratedGuardCourse implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Review the provided course blueprint for missing CTAs, accessibility violations, and policy blockers. Respond with JSON {"summary": string, "violations": string[], "status": "passed"|"failed"}.`;
    // Basic interpolation
    if (payload) {
