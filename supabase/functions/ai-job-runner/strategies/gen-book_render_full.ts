...
export class GeneratedBookRenderFull implements JobExecutor {
  async execute(context: JobContext): Promise<any> {
    const { payload } = context;
    
    // 1. Interpolate Prompt
    let prompt = `Enqueue a full book render job for a canonical book version. Return JSON {"run_id": string}.`;
    // Basic interpolation
    if (payload) {
