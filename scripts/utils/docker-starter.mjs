#!/usr/bin/env node
/**
 * Docker Desktop Auto-Starter for Windows
 * 
 ...
 * @returns {boolean} True if started successfully or already running
 */
 ...
 * @param {boolean} options.autoStart - Automatically start Docker if not running (default: true)
 * @param {boolean} options.silent - Suppress console output except errors (default: false)
 * @returns {Promise<boolean>} True if Docker is running or was started successfully
 */
...
if (import.meta.url === `file://${process.argv[1]}`) {
  (async () => {
    const success = await ensureDockerRunning();
