/**
 * Game Logic Tests (Jest version)
 * Tests for the pure game logic functions.
 */

import { nextVariant, resolveOnWrong } from '@/lib/gameLogic';
...
describe('resolveOnWrong', () => {
  ...
  it('should return current item if no variant', () => {
    const noVariantItem: CourseItem = {
      ...baseItem,
      clusterId: 'cluster-1',
      variant: '',
    };
    const pool = [noVariantItem];
    const variantMap = new Map<string, number>();

    const result = resolveOnWrong(noVariantItem, pool, mockCatalog, variantMap);

    expect(result.itemToEnqueue).toBe(noVariantItem);
    expect(result.nextVariantNum).toBeUndefined();
  });
  ...
  it('should find variant when multiple clusters exist', () => {
    const cluster2Item: CourseItem = {
      ...baseItem,
      id: 10,
      clusterId: 'cluster-2',
      variant: '1',
    };
    const cluster2Variant2: CourseItem = {
      ...baseItem,
      id: 11,
      clusterId: 'cluster-2',
      variant: '2',
    };
    const multiClusterCatalog: Course = {
      ...mockCatalog,
      items: [...mockCatalog.items, cluster2Item, cluster2Variant2],
    };
    const pool = [cluster2Item];
    const variantMap = new Map<string, number>();

    const result = resolveOnWrong(cluster2Item, pool, multiClusterCatalog, variantMap);
