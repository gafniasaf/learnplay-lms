...
import { compileSkeletonToCanonical, validateBookSkeleton, type BookSkeletonV1 } from "../bookSkeletonCore.js";
...
async function callEdgeAsAgent(opts: { orgId: string; path: string; body: unknown }) {
  ...
  if (!res.ok) {
    const msg = typeof (json as any)?.error?.message === "string" ? (json as any).error.message : `Edge call failed (${res.status})`;
    throw new Error(msg);
  }
  return json;
}
...
function buildScaffoldSkeleton(opts: {
  ...
  const { bookId, bookVersionId, title, level, language, chapterCount, promptPackId, promptPackVersion } = opts;
...
export class BookGenerateFull implements JobExecutor {
  async execute(context: JobContext): Promise<unknown> {
    ...
    const mode = requireEnum((p as any).mode, ["create", "existing"] as const, "mode");
    const bookId = requireString(p, "bookId");
    const language = requireString(p, "language");
    const level = requireEnum((p as any).level, ["n3", "n4"] as const, "level");
    ...
    const promptPackVersionRaw = (p as any).promptPackVersion;
    ...
    if ((saveRes as any)?.ok !== true) throw new Error("Failed to save skeleton scaffold");
