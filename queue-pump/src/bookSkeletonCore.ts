...
export function validateBookSkeleton(
  ...
  if (!isPlainObject(meta)) {
    ...
    const bookId = asString((meta as any).bookId).trim();
    const bookVersionId = asString((meta as any).bookVersionId).trim();
    const title = asString((meta as any).title).trim();
    const level = asString((meta as any).level).trim();
    const language = asString((meta as any).language).trim();
    const schemaVersion = asString((meta as any).schemaVersion).trim();
  ...
  const chapters = (raw as any).chapters;
  ...
  const validateBlock = (b: unknown, pathBase: string[]) => {
    ...
    if (t === "paragraph") {
      ...
      if (!String(basisHtml || "").trim())
        push("warning", "empty_paragraph", "paragraph.basisHtml is empty", pathBase.concat(["basisHtml"]));
    ...
    for (let ci = 0; ci < chapters.length; ci++) {
      ...
      if (typeof recapRaw !== "undefined" && recapRaw !== null) {
        if (!isPlainObject(recapRaw)) {
          ...
          const validateItems = (key: string, itemsRaw: unknown, itemValidator: (item: unknown, itemPath: string[]) => void) => {
...
export function compileSkeletonToCanonical(sk: BookSkeletonV1): unknown {
  const meta = isPlainObject((sk as any)?.meta) ? ((sk as any).meta as any) : {};
